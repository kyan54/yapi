...
class exportController extends baseController {
  ...
  async handleListClass(pid, status, catid) {
    ...
    if (catid && catid !== 'all') {
      result = result.filter(item => String(item._id) === String(catid));
    }
  ...
  async exportData(ctx) {
    ...
    try {
      ...
      const list = await this.handleListClass(pid, status, catid);

      if (type === 'word') {
        ctx.body = yapi.commons.resReturn(null, 400, '不再支持 Word 导出');
        return;
      }

      switch (type) {
        case 'markdown': {
          payload = await createMarkdown.call(this, list, false);
          ctx.body = payload;
        ...
        case 'json': {
          payload = JSON.stringify(this.handleExistId(list), null, 2);
          ctx.body = payload;
        ...
        default: {
          payload = await createHtml.call(this, list);
          ctx.body = payload;
          ctx.type = 'text/html';
          ctx.charset = 'utf-8';
    ...
    async function createHtml(list) {
      let markdownSource = await createMarkdown.call(this, list, true);
      ...
      let htmlContent = unescape(markdown.render(markdownSource));
      let left;
    ...
    function createHtml5(left, content) {
      return `<!DOCTYPE html>
      <html>
      <head>
    ...
    function createMarkdown(list, isToc) {
      ...
      try {
        markdownTemplate += md.createProjectMarkdown(curProject, wikiData);
        markdownTemplate += md.createClassMarkdown(curProject, list, isToc);
