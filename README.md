...
### 由于原始仓库不更新了，本仓库继续在原版本上做改动

* 现在由 上海营联信息技术有限公司 进行开源维护
* 本版本基于原仓库的v1.12.0版本进行改动

#### 1. 接口请求支持两种模式
* 在接口运行页和测试集合页增加了“请求方式”开关
* 支持“服务端代发请求”和“浏览器直接请求”两种模式
* 浏览器模式下不再依赖 `cross-request` 插件，但目标接口需要支持 CORS
* 服务端模式下由 YApi 后端代理请求，适合跨域或需要由服务器网络环境访问目标接口的场景

#### 2. 去掉插件安装依赖
* 移除了接口调试页和测试集合页中关于安装 `cross-request` 插件的提示
* 现在浏览器可以直接使用，不再强制要求安装插件

#### 3. 默认请求头优化
* 新建接口时会默认补充请求头 `Content-Type: application/json`
* 编辑页默认展示的请求头也会预置为 `Content-Type: application/json`

#### 4. Node.js 24 运行适配
* 已适配 Node.js v24 运行当前项目
* 已验证本地 MongoDB 无密码场景下可以正常启动服务
* 已修复静态资源返回类型异常导致页面下载文件或样式丢失的问题
