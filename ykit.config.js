...
// Add polyfills for Node 22+ compatibility
...
var yapiConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
...
function initPlugins(configPlugin) {
  configPlugin = yapiConfig.plugins;
  ...
  plugins: [
    {
      ...
      options: {
        ...
        exclude: isWin ? /(tui-editor|node_modules\\(?!_?(yapi-plugin|json-schema-editor-visual|fs-extra|jsonfile|universalify|nodemailer|libmime|buildmail|addressparser)))/ : /(tui-editor|node_modules\/(?!_?(yapi-plugin|json-schema-editor-visual|fs-extra|jsonfile|universalify|nodemailer|libmime|buildmail|addressparser)))/
      }
    }
  ],
